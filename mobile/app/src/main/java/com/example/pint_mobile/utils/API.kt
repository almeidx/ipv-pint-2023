...
fun listaBeneficios(
    list: ArrayList<Beneficio>,
    allList: ArrayList<Beneficio>,
    adapter: BeneficiosActivity.BeneficioAdapter,
    ctx: Context,
    admin: Boolean = false
) {
    ...
    val request = JsonArrayRequestWithCookie(ctx, Request.Method.GET,
        "$API_URL/beneficios${if (admin) "?admin" else ""}", null, { response ->
            try {
...
fun listarVagasUser(
    list: ArrayList<vagaCandidatada>,
    allList: ArrayList<vagaCandidatada>,
    adapter: VagasCandidatadasActivity.VagaCandidatadaAdapter,
    ctx: Context
) {
    val queue = Volley.newRequestQueue(ctx)
...
fun listaVagas(
    list: ArrayList<Vaga>,
    allList: ArrayList<Vaga>,
    adapter: VagasActivity.VagaAdapter,
    ctx: Context
) {
    val queue = Volley.newRequestQueue(ctx)
...
fun listaAreas(
    list: ArrayList<AreaNegocio>,
    allList: ArrayList<AreaNegocio>,
    adapter: AreasNegocioActivity.AreasAdapter,
    ctx: Context
) {
    val queue = Volley.newRequestQueue(ctx)
...
fun listaTipoProjeto(
    list: ArrayList<TipoProjeto>,
    allList: ArrayList<TipoProjeto>,
    adapter: AdminTiposProjetoActivity.ProjetosAdapter,
    ctx: Context
) {
    val queue = Volley.newRequestQueue(ctx)
...
fun listaNegociosUser(
    ...
    val request =
        JsonArrayRequestWithCookie(ctx, Request.Method.GET, "$API_URL/negocios", null, { response ->
            try {
                for (i in 0 until response.length()) {
                    val rawNegocio = response.getJSONObject(i)
...
fun listaUtilizadores(
    list: ArrayList<Utilizador>,
    allList: ArrayList<Utilizador>,
    adapter: AdminUtilizadoresActivity.UtilizadorAdapter,
    ctx: Context
) {
    val queue = Volley.newRequestQueue(ctx)
    val request = JsonArrayRequestWithCookie(
...
fun listaClientes(
    list: ArrayList<Cliente>,
    allList: ArrayList<Cliente>,
    adapter: AdicionarClienteNegocioActivity.ClienteAdapter,
    ctx: Context
) {
    val queue = Volley.newRequestQueue(ctx)

    val request =
        JsonArrayRequestWithCookie(ctx, Request.Method.GET, "$API_URL/clientes", null, { response ->
            try {
                for (i in 0 until response.length()) {
...
fun listaContactosCliente(
    list: ArrayList<Contacto>,
    allList: ArrayList<Contacto>,
    adapter: SelectContactoClienteNegocioActivity.ContactosAdapter,
    ctx: Context,
    id: Int
) {
    val queue = Volley.newRequestQueue(ctx)
...
fun listarAreasNegocio(
    list: ArrayList<AreaNegocio>,
    adapter: CriarNegocioActivity.AreaNegocioAdapter,
    ctx: Context,
    callback: () -> Unit
) {
    val queue = Volley.newRequestQueue(ctx)
...
fun listarTiposProjeto(
    list: ArrayList<TipoProjeto>,
    adapter: CriarNegocioActivity.TipoProjetoAdapter,
    ctx: Context,
    callback: () -> Unit
) {
    val queue = Volley.newRequestQueue(ctx)
...
fun listarCentroTrabalho(
    list: ArrayList<CentroTrabalho>,
    adapter: EditNegocioActivity.CentroTrabalhoAdapter,
    ctx: Context,
    callback: () -> Unit
) {
    val queue = Volley.newRequestQueue(ctx)
...
fun listaIdeias(
    list: ArrayList<Ideia>,
    allList: ArrayList<Ideia>,
    adapter: AdminIdeiasActivity.IdeiaAdapter,
    ctx: Context
) {
    val queue = Volley.newRequestQueue(ctx)

    val request =
        JsonArrayRequestWithCookie(ctx, Request.Method.GET, "$API_URL/ideias", null, { response ->
            try {
                for (i in 0 until response.length()) {
...
fun listaCandidaturas(
    list: ArrayList<Candidatura>,
    allList: ArrayList<Candidatura>,
    adapter: AdminCandidaturasActivity.CandidaturaAdapter,
    ctx: Context,
    admin: Boolean = false
) {
    val queue = Volley.newRequestQueue(ctx)
    ...
    val request = JsonArrayRequestWithCookie(
        ctx,
        Request.Method.GET,
        "$API_URL/candidaturas?${if (admin) "admin" else ""}",
        null,
        { response ->
            try {
                for (i in 0 until response.length()) {
...
fun listaMensagens(
    list: ArrayList<Mensagem>,
    allList: ArrayList<Mensagem>,
    adapter: AdminMensagensActivity.MensagemAdapter,
    ctx: Context
) {
    val queue = Volley.newRequestQueue(ctx)
...
fun listaReunioes(
    list: ArrayList<Reuniao>,
    allList: ArrayList<Reuniao>,
    adapter: AdminReunioesActivity.ReuniaoAdapter,
    ctx: Context,
    admin: Boolean = false
) {
    val queue = Volley.newRequestQueue(ctx)
    ...
    val request = JsonArrayRequestWithCookie(
        ctx,
        Request.Method.GET,
        "$API_URL/reunioes?${if (admin) "admin" else ""}",
        null,
        { response ->
            try {
                for (i in 0 until response.length()) {
...
fun listaNotificacoes(
    list: ArrayList<Notificacao>,
    adapter: NotificacoesActivity.NotificacaoAdapter,
    ctx: Context
) {
    val queue = Volley.newRequestQueue(ctx)
...
fun listarCentrosTrabalho(
    list: ArrayList<CentroTrabalho>,
    allList: ArrayList<CentroTrabalho>,
    adapter: CentroTrabalhoActivity.CentroTrabalhoAdapterX,
    ctx: Context
) {
    val queue = Volley.newRequestQueue(ctx)

    val request =
        JsonArrayRequestWithCookie(ctx, Request.Method.GET, "$API_URL/centros-de-trabalho", null,
            { response ->
                for (i in 0 until response.length()) {
...
fun listarNotasReuniao(
    ...
    allList: ArrayList<NotaReuniao>,
    adapter: EditarNotaEntrevistaActivity.NotaReuniaoAdapter,
    id: Int,
    ctx: Context
) {
    val queue = Volley.newRequestQueue(ctx)
    ...
    val request =
        JsonObjectRequestWithCookie(ctx, Request.Method.GET, "$API_URL/reunioes/$id/notas", null,
            { response ->
...
fun listaEventos(
    list: ArrayList<Evento>,
    allList: ArrayList<Evento>,
    adapter: CalendarioActivity.EventoAdapter,
    ctx: Context
) {
    val queue = Volley.newRequestQueue(ctx)

    val request =
        JsonArrayRequestWithCookie(ctx, Request.Method.GET, "$API_URL/events", null, { response ->
            try {
                for (i in 0 until response.length()) {
    ...
    val request = JsonObjectRequestWithSessionId(Request.Method.POST, "$API_URL/auth/email", body,
        ...
        { error ->
            if (error.networkResponse?.statusCode == 403) {
                Toast.makeText(ctx, "Conta desativada por um administrador", Toast.LENGTH_SHORT)
                    .show()
            } else {
...
fun googleLogin(email: String, id: String, name: String, ctx: Context, onError: () -> Unit) {
    ...
    val request = JsonObjectRequestWithSessionId(Request.Method.POST,
...
fun facebookLogin(email: String, id: String, name: String, ctx: Context, onError: () -> Unit) {
    ...
    val request = JsonObjectRequestWithSessionId(Request.Method.POST,
        "$API_URL/auth/facebook/callback/mobile",
        body,
        { response ->
            val cookie = response.getString("cookie")
            val data = response.getJSONObject("data")
...
fun signup(nome: String, email: String, password: String, ctx: Context) {
    ...
    val request =
        object : JsonObjectRequestWithSessionId(
            Method.POST, "$API_URL/auth/register", body,
            { response ->
...
fun getUserInfo(cookie: String, ctx: Context) {
    ...
    val request = object : JsonObjectRequestWithSessionId(
        Method.GET,
        "$API_URL/auth/user",
        null,
...
fun deleteBeneficio(id: Int, ctx: Context) {
    ...
    val request = object : JsonObjectRequestWithCookie(
        ctx,
        Method.DELETE,
        "$API_URL/beneficios/$id",
        null,
        Response.Listener { response ->
...
fun createClient(
    nome: String,
    clienteNome: ArrayList<String> = ArrayList(),
    clienteIds: ArrayList<Int> = ArrayList(),
    ctx: Context
) {
    val queue = Volley.newRequestQueue(ctx)
...
fun createContactoClient(
    idCliente: Int,
    type: Int,
    contacto: String,
    clientNames: ArrayList<String>,
    clienteIds: ArrayList<Int>,
    contactoIds: ArrayList<Int>,
    contactoNames: ArrayList<String>,
    ctx: Context
) {
    val queue = Volley.newRequestQueue(ctx)
    ...
    val request = object : JsonObjectRequestWithCookie(
        ctx,
        Method.POST,
        "$API_URL/clientes/$idCliente/contactos",
        body,
        Response.Listener { response ->
            Toast.makeText(ctx, "Cliente criado com sucesso!", Toast.LENGTH_LONG).show()
...
fun createNotaReuniao(idCandidatura: Int, nota: String, ctx: Context) {
    ...
    val request = object : JsonObjectRequestWithCookie(
        ctx,
        Method.POST,
        "$API_URL/reunioes/$idCandidatura/notas",
        body,
        Response.Listener { response ->
...
fun desativarUser(idUser: Int, disabled: Boolean, user: Int?, ctx: Context, callback: () -> Unit) {
    ...
    val request = object : JsonObjectRequestWithCookie(
        ctx,
        Method.PATCH,
        "$API_URL/utilizadores/$idUser/disable",
        body,
        Response.Listener { response ->
...
fun mudarPasswordPerfil(passwordAntiga: String, passwordNova: String, ctx: Context) {
    ...
    val request = object : JsonObjectRequestWithCookie(
        ctx,
        Method.PATCH,
        "$API_URL/utilizadores/@me/password",
        body,
        Response.Listener { response ->
...
fun deleteAreaNegocio(idAreaNegocio: Int, ctx: Context) {
    ...
    val request = object : JsonObjectRequestWithCookie(
        ctx,
        Method.DELETE,
        "$API_URL/areas-de-negocio/$idAreaNegocio",
        body,
        Response.Listener { response ->
        ...
        Response.ErrorListener { error ->
            Toast.makeText(
                ctx,
                "Área de Negócio em uso, impossivel de ser apagada!",
                Toast.LENGTH_LONG
            ).show()
            error.printStackTrace()
        }) {
...
fun deleteTipoProjeto(idTipoProjeto: Int, ctx: Context) {
    ...
    val request = object : JsonObjectRequestWithCookie(
        ctx,
        Method.DELETE,
        "$API_URL/tipos-projeto/$idTipoProjeto",
        body,
        Response.Listener { response ->
        ...
        Response.ErrorListener { error ->
            Toast.makeText(
                ctx,
                "Tipo de Projeto em uso, impossivel de ser apagado!",
                Toast.LENGTH_LONG
            ).show()
            error.printStackTrace()
        }) {
...
fun deleteCentroTrabalho(idCentroTrabalho: Int, ctx: Context) {
    ...
    val request = object : JsonObjectRequestWithCookie(
        ctx,
        Method.DELETE,
        "$API_URL/centros-de-trabalho/$idCentroTrabalho",
        null,
        Response.Listener { response ->
        ...
        Response.ErrorListener { error ->
            Toast.makeText(
                ctx,
                "Centro de Trabalho em uso, impossivel de ser apagado!",
                Toast.LENGTH_LONG
            ).show()
            error.printStackTrace()
        }) {
...
fun editarUtilizadorAtual(cv: String, ctx: Context) {
    ...
    val request =
        JsonObjectRequestWithCookie(ctx, Request.Method.PATCH, "$API_URL/utilizadores/@me", body,
            { response ->
...
fun candidatarVaga(idVaga: Int, refEmail: String?, ctx: Context) {
    ...
    val request = JsonObjectRequestWithCookie(ctx,
        Request.Method.POST,
        "$API_URL/vagas/$idVaga/candidatar",
        body,
        { response ->
...
fun forgetPassword(email: String, ctx: Context) {
    ...
    val request = JsonObjectRequestWithCookie(ctx,
        Request.Method.POST,
        "$API_URL/utilizadores/esqueceu-password",
        body,
        { response ->
...
fun enviarCodigoConfirmacao(userID: Int, email: String, ctx: Context, onError: () -> Unit) {
    ...
    val request =
        JsonObjectRequestWithSessionId(Request.Method.POST, "$API_URL/auth/validate", body,
            { response ->
                val cookie = response.getString("cookie")
                val data = response.getJSONObject("data")
                val user = data.getJSONObject("user")
...
fun reenviarCodigoConfirmacao(userID: Int, ctx: Context) {
    ...
    val request =
        JsonObjectRequestWithSessionId(Request.Method.POST, "$API_URL/auth/validate/retry", body,
            { response ->
                Toast.makeText(ctx, "Email enviado com sucesso!", Toast.LENGTH_LONG).show()
            },
            { error ->
